...
export class TagService extends HandleMessageError {
  ...
  createTag(tag: any): Observable<Tag> {
    return this.http.post<Tag>(`${environment.serverURL}/tags`, tag)
      .pipe(catchError(this.handleError));
  }

  deleteTag(id: string): Observable<any> {
    return this.http.delete(`${environment.serverURL}/tags/${id}`)
      .pipe(catchError(this.handleError));
  }

  getTagsByTaskId(taskId: string): Observable<Tag[]> {
    return this.http.get<Tag[]>(`${environment.serverURL}/tags/task/${taskId}`)
      .pipe(catchError(this.handleError));
  }
